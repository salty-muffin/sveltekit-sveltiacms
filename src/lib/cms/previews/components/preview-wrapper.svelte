...
<div class="preview__wrapper" bind:clientWidth={viewportWidth}>
	<div class="preview__controls">
		...
		<button type="button" disabled={step === DEFAULT_STEP} onclick={() => (step = DEFAULT_STEP)}
			>⟲</button
		>
		<span class="preview__zoom"
			>{Math.round(zoom * 100)}% · {Math.round(viewportWidth / zoom)}px</span
		>
	</div>
	<span class="preview__note">This preview is not 100% representative for the finished page.</span>
	<div class="preview__frame" style:zoom>
		{@render children()}
	</div>
</div>
